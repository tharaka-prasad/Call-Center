...
export default function Index({ calls, filters, districts, reasons, products, models }: Props) {
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            ...
            case "pending":  return { bg: "bg-amber-50 text-amber-600 border-amber-200",       dot: "bg-amber-400",  label: "Pending"  };
            case "close":    return { bg: "bg-blue-50 text-blue-600 border-blue-200",           dot: "bg-blue-400",   label: "Closed"   };
            case "fail":     return { bg: "bg-red-50 text-red-600 border-red-200",              dot: "bg-red-400",    label: "Failed"   };
            default:         return { bg: "bg-gray-100 text-gray-600 border-gray-200",          dot: "bg-gray-400",   label: status     };
        }
    };
    ...
    const downloadExcel = () => {
        const rows = calls.map((c) => ({
            ...
            Model:    c.productModel?.productModel  ?? "",
    ...
    const downloadPDF = () => {
        ...
        autoTable(doc, {
            ...
            columnStyles:       { 6: { cellWidth: 18 }, 7: { cellWidth: 22 } },
            ...
            header={
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-base font-bold text-gray-900 leading-tight">Call Reports</h2>
                            <p className="text-xs text-gray-400">Filter and export call records</p>
                        </div>
                    </div>
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-5">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        {[
                            ...
                            { label: "Failed",        value: failed,    color: "text-red-600",     bar: "bg-red-400",     barBg: "bg-red-50"     },
                        ].map((s) => (
                                                ...
                                                <tr key={call.id} className="hover:bg-gray-50/70 transition-colors">
                                                    <td className="px-4 py-3">
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <span className="font-semibold text-gray-800">{call.customerName}</span>
                                                        </div>
                                                    </td>
